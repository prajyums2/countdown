...
import type { Station, Milestone, JourneyConfig, SheetData, ProfileId } from "@/lib/types";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        setUnreadCount(snaps.filter((s) => s.status === "unread").length);
      } catch {}
    };
    poll();
  ...
  const overlayOpen = showCamera || showChat || showAdmin || showAdminPin;
  ...
  return (
    <>
      ...
      {ready && (
        <main className="min-h-screen bg-[#FFF9F6] bg-paper-texture px-4 pt-8 pb-24 md:py-16 relative">
